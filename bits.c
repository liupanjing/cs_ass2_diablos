...
Bool bitIsSet(Bits b, int position) {
    ...
    //TODO
    return (b->bitstring[position / 8] & (1 << (position % 8))) ? TRUE : FALSE;
}
...
void setAllBits(Bits b) {
    assert(b != NULL);
    //TODO
    for (int i = 0; i < b->nbytes; i++) b->bitstring[i] = 255;
}
...
void unsetAllBits(Bits b) {
    assert(b != NULL);
    //TODO
    for (int i = 0; i < b->nbytes; i++) b->bitstring[i] = 0;
}
...
void andBits(Bits b1, Bits b2) {
    ...
    assert(b1->nbytes == b2->nbytes);
    //TODO
    for (int i = 0; i < b1->nbytes; i++) b1->bitstring[i] = (b1->bitstring[i] & b2->bitstring[i]);
}
